...
const Browse = () => {
  ...
  useEffect(() => {
    ...
    makeApiCall(`${BASE_URL}/genre/movie/list?api_key=${API_KEY}`).then(
      (response) => {
    ...
    makeApiCall(`${BASE_URL}/genre/tv/list?api_key=${API_KEY}`).then(
      (response) => {
    ...
    // Movie API Calls
    makeApiCall(
      `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=${ACTION_MOVIE_VAL}`
    ).then((response) => setActionMovies(refineMovies(response.results)));

    makeApiCall(
      `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=${COMEDY_MOVIE_VAL}`
    ).then((response) => setComedyMovies(refineMovies(response.results)));

    makeApiCall(
      `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_genres=${ADVENTURE_MOVIE_VAL}`
    ).then((response) => setAdventureMovies(refineMovies(response.results)));

    // TV API Calls
    makeApiCall(
      `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=${ANIMATION_TV_VAL}`
    ).then((response) => setAnimationTVShows(refineTVShows(response.results)));

    makeApiCall(
      `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=${COMEDY_TV_VAL}`
    ).then((response) => setComedyTVShows(refineTVShows(response.results)));

    makeApiCall(
      `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_genres=${SCI_FI_FANTASY_VAL}`
    ).then((response) =>
      setSciFiFantasyTVShows(refineTVShows(response.results))
    );

    // People API Calls
    makeApiCall(`${BASE_URL}/trending/person/day?api_key=${API_KEY}`).then(
      (response) => {
        console.log(response);
